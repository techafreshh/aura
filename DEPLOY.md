...
- **Tables:**
  - `users` — `id`, `email` (unique), `name`, `avatar_url`, `provider` (`google` | `github` | `email`), `provider_id`, `role` (last-picked mode: `""` | `candidate` | `recruiter`; `admin` via ADMIN_EMAIL promotion), `is_recruiter` (one-way recruiter capability grant — the migration backfills `1` for users whose `role` was already `recruiter`), `password_hash` (null for OAuth-only accounts), `email_verified`, `verification_token_hash` + `verification_token_expires_at`, `reset_token_hash` + `reset_token_expires_at`, `created_at`, `last_login_at`. Only SHA-256 hashes of email tokens are stored — raw tokens exist solely inside email links.
  ...
  - `candidate_profiles` — job-board profile, one row per user (created lazily): `headline`, `location`, `summary`, `skills_json`, `experience_json`, `education_json`, `linkedin_url`/`github_url`/`portfolio_url`, `resume_stored` + `resume_uploaded_at` (the PDF itself lives in MinIO under `resumes/{user_id}/`), timestamps. Added by the dual-roles revision.
  - `recruiter_profiles` — company branding shown on invite previews: `company_name`, `job_title`, `company_website`, `company_location`, timestamps. Added by the dual-roles revision.
