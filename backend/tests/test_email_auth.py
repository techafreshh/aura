...
from db.crud import get_user_by_email, store_verification_token, upsert_user
...
class TestRegister:
    ...
    async def test_register_duplicate_oauth_email_points_to_provider(self, ip_headers, sent_emails):
        email = _unique_email()
        async with async_session() as db:
            db.add(User(email=email, name="OAuth User", provider="google", provider_id="g-1", email_verified=True))
            await db.commit()
        async with _client() as ac:
            resp = await ac.post(
                "/auth/register",
...
class TestLogin:
    ...
    async def test_login_oauth_account_without_password(self, ip_headers):
        email = _unique_email()
        async with async_session() as db:
            db.add(User(email=email, name="OAuth User", provider="github", provider_id="gh-1", email_verified=True))
            await db.commit()
        async with _client() as ac:
            resp = await ac.post(
                "/auth/login",
...
class TestPasswordReset:
    ...
    async def test_forgot_silent_for_oauth_only_and_unknown(self, ip_headers, sent_emails):
        email = _unique_email()
        async with async_session() as db:
            db.add(User(email=email, name="OAuth User", provider="google", provider_id="g-2", email_verified=True))
            await db.commit()
        async with _client() as ac:
...
class TestUpsertCreatedFlag:
    ...
    async def test_created_flag_true_only_on_first_insert(self):
        email = _unique_email()
        async with async_session() as db:
            _, created_first = await upsert_user(
                db, email=email, name="A", provider="google", provider_id="p-1"
            )
            _, created_second = await upsert_user(
